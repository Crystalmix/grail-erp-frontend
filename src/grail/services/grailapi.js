...
export default ($http, configs, auth_status, $state, $exceptionHandler, xero: XeroConst) => {
  ...
  api.updateOrganization = (id, data) => api.request('PUT', `/organizations/${id}/`, data)
  ...
  api.updateContact = (id, data) => api.request('PUT', `/contacts/${id}/`, data)
  api.addContact = (data) => api.request('POST', '/contacts/', data)
  api.deleteContactLocation = (contact_id, location_id) => api.request('Delete', `/contacts/${contact_id}/delete_location/${location_id}/`)
  ...
  api.updateWarehouse = (id, data) => api.request('PUT', `/warehouses/${id}/`, data)
  api.addWarehouse = (data) => api.request('POST', '/warehouses/', data)

  // Products
  api.getProducts = () => api.request('GET', '/products/')
  api.updateProduct = (id, data) => api.request('PUT', `/products/${id}/`, data)
  api.addProduct = (data) => api.request('POST', '/products/', data)
  api.deactivateProductItem = (product_id, item_id) => api.request('POST', `/products/${product_id}/deactivate_item/${item_id}/`)
  api.activateProductItem = (product_id, item_id) => api.request('POST', `/products/${product_id}/activate_item/${item_id}/`)

  // Purchases
  api.getPurchases = () => api.request('GET', `/invoices/?Type=${xero.invoice.purchase_type}`)
  api.getPurchase = (id) => api.request('GET', `/invoices/${id}/`)
  api.updatePurchase = (id, data) => api.request('PUT', `/invoices/${id}/`, data)
  api.addPurchase = (data) => api.request('POST', '/invoices/', data)
  api.deleteInvoiceLineItem = (invoice_id, item_id) => api.request('Delete', `/invoices/${invoice_id}/delete_item/${item_id}/`)
  ...
  api.getSale = (id) => api.request('GET', `/invoices/${id}/`)
  api.updateSale = (id, data) => api.request('PUT', `/invoices/${id}/`, data)
